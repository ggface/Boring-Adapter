...
import io.github.ggface.boring.adapter.presentation.adapter.legacy.RendererModel
 ...
 */
class ExampleLegacyRendererView(
    private val onClickListener: (productId: String) -> Unit
) : RendererView<ExampleLegacyRendererView.Model, ExampleLegacyRendererView.ViewHolder>(Model.TYPE) {
    ...
    class ViewHolder(
        ...
        fun bindView(model: Model) = with(binding) {
            productId = model.id
            binding.idTextView.text = "id=${model.id}"
        }
    }
}